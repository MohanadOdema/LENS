...
def get_values(x):
    ...
    i = 0
    while i<5:
        arch_param['f%s' %(i+1)] = int(x[i*4])
        arch_param['k%s' %(i+1)] = int(x[i*4 + 1])
        arch_param['l%s' %(i+1)] = int(x[i*4 + 2])
        arch_param['p%s' %(i+1)] = int(x[i*4 + 3])
        i = i+1
    arch_param['fc1'] = int(x[20])
    arch_param['fc2'] = int(x[21]) 
    ...
    if arch_param['l4'] == 3:
        ...
        convD3 = layers.Conv2D(arch_param['f4'], (arch_param['k4'], arch_param['k4']), (1, 1), kernel_initializer='he_uniform', padding='same', data_format="channels_last")(convD2)
    ...
    output = layers.Dense(num_classes, kernel_initializer='he_uniform')(fc2)
