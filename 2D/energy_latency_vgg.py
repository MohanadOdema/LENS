...
def evaluations_dev():	#Call Berken's models
	...
	latency_conv = Call_CNN_Latency([Size[0], (arch_param['k1']**2)*arch_param['f1']]) + Call_Norm_Latency(Size[1]/4) + Call_Relu_Latency(Size[1]/4) #latency for 3 layers conv, batch, activation
	...
	if arch_param['p1'] == 1:
		...
		latency_pool = Call_Pool_Latency([Size[1]/4, Size[2]/4])
	else:
		power_pool = 0
		latency_pool = 0
	...
	while i<5:
		...
		latency_conv = Call_CNN_Latency([(Size[2*i]/4), (arch_param['k%s' %(i+1)]**2)*arch_param['f%s' %(i+1)]]) + Call_Norm_Latency(Size[2*i + 1]/4) + Call_Relu_Latency(Size[2*i + 1]/4)
		...
		if arch_param['p%s' %(i+1)] == 1:
			power_pool = Call_Pool_Power([Size[2*i + 1]/4, Size[2*i + 2]/4])
			latency_pool =Call_Pool_Latency([Size[2*i + 1]/4, Size[2*i + 2]/4])
		else:
			power_pool = 0
			latency_pool = 0
		Latency_dev.append(latency_pool) #mseconds
...
def identify_splits():
	...
	while i<len(Size)-1:		#Because we already include the all-edge solution separately
		if Size[0]<Size[i]:		#Append only layers who offer size gains over the all-cloud solution
			...
			index_list.append(i)
			i = i+1
	index_list.append(13)		#The all-edge solution
...
def arrange_splits(tu, ALPHA, BETA):
	...
	return index_list_sorted_latency, index_list_sorted_energy, sorted(Cumulative_latency), sorted(Cumulative_energy)
...
def evaluate(x, tu, technology):
	...
	#print(len(Size), len(Energy_dev), len(Latency_dev))
	...
	identify_splits()
	index_latency, index_energy, values_latency, values_energy = arrange_splits(tu, ALPHA, BETA)

	return index_latency, index_energy, values_latency, values_energy
	#return 0,0,0,0
